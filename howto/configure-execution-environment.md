...
All configuration models enforce Jakarta Bean Validation rules at startup. In addition, `RestartStrategyConfiguration` performs strict fail-fast validation:
* If an incompatible sub-configuration block (e.g. `fixed-delay`) is specified for another strategy `type` (e.g. `EXPONENTIAL_DELAY`), an `InvalidRestartStrategyConfigurationException` is thrown at startup.
